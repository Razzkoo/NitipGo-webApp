...
const stats = [
  ...
  { label: "Pendapatan Bulan Ini", value: 450000, prefix: "Rp ", icon: TrendingUp, color: "text-accent", bgColor: "bg-accent/10", },
...
const handleAcceptOrder = (orderId: string) => {
  console.log("Order diterima:", orderId);
  ...
  setUpcomingTrips((prevTrips) =>
    prevTrips.map((trip) =>
      trip.id === 1
        ? {
            ...trip,
            orders: trip.orders + 1,
            capacityPercent: Math.min(trip.capacityPercent + 10, 100),
          }
        : trip
    )
  );

  // 2. hapus order dari pending
  setPendingOrders((prev) =>
    prev.filter((order) => order.id !== orderId)
  );

  // 3. optional: pindah halaman
  navigate("/traveler/orders");
};

const handleRejectOrder = (orderId: string) => {
  console.log("Order ditolak:", orderId);
};
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <Button variant="soft" size="sm" asChild>
                        <Link to={`/traveler/trip/:id/manage`}>Kelola</Link>
                      </Button>
                    </div>
                  </motion.div>
                ))}
